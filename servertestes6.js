...
// Game Functions
const generateCard = () => {
  const card = Array.from({length: 24}, () => Math.floor(Math.random() * 100));
  return [...new Set(card)];
};

const checkBingo = (card, draftedNumbers) => {let card2D = [
    card.slice(0, 5),
    card.slice(5, 10),
...
const waitForReadyPlayers = () => {return new Promise((resolve) => {
...
const draftNumber = () => {let draftedNumber;
    do {
...
// Server Functions
const login = (call) => {const { username } = call.request;
...
const ready = async (call, callback) => {const { username, token } = call.request;
...
const play = (call) => {const { username, token } = call.request;
const player = players.find((p) => p.username.toLowerCase() === username.toLowerCase() && p.token === token);
if (player) {
  player.call = call;
  startGame();
...
const checkWin = (call, callback) => {const { username, token } = call.request;
const player = players.find((p) => p.username.toLowerCase() === username.toLowerCase() && p.token === token);
if (player) {
